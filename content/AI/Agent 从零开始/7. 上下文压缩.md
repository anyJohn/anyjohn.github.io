...
Agent 真需要那个细节的时候，它会用工具重新获取——再 `ls` 一次，再 `read` 一遍文件。这是一种**人类行为代偿**：你不会记得两周前每封邮件的全文，但你知道去哪搜。Agent 有工具，就像你有搜索栏——丢了的东西随时能找回来。
...
压缩就是教它**抓大放小**——把笔记本里的流水账撕掉，只留几行要点和正在写的那一页。
