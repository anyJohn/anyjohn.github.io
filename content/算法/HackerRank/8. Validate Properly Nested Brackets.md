...
Origin: [Validate Properly Nested Brackets](https://www.hackerrank.com/contests/software-engineer-prep-kit/challenges/validate-properly-nested-brackets)
...
```python
def areBracketsProperlyMatched(code_snippet: str) -> int:
    stack = []
    start = '({['
    mapping = {
        ")": "(",
    ...
    for char in code_snippet:
        if char in mapping:
            ...
            # 只有明确的左括号才入栈，忽略其他字符（数字、字母等）
            stack.append(char)
            
    # 栈为空说明全部匹配，返回 1，否则返回 0
